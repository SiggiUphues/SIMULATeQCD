...
void measureHadrons<floatT, onDevice, HaloDepth, HaloDepthSpin, Source, NStacks, compHISQ, compstdStag>::invert_StaggeredType_Dslash<StaggeredTypeAction , n_color>(
    ...
    floatT cg_residue = _individual_cg_residues[ mass_index ];
    int cg_max_iter = _individual_cg_max_iters[ mass_index ];
    ...
    prop_containers[mass_index].template iterateOverEvenBulkAtStack<color0>(
    ...
    h_Go = d_Go;
    //! same for odd part
    prop_containers[mass_index].template iterateOverOddBulkAtStack<color0>(
    ...
    cg.invert(dslash, d_Ge, d_Se, cg_max_iter, cg_residue); //! this takes up most of the computation time
    h_Ge = d_Ge;
    prop_containers[mass_index].template iterateOverEvenBulkAtStack<color1>(
            returnSpinor<floatT, false, Even, HaloDepthSpin, NStacks>(h_Ge));
    dslash.Dslash(d_Go, d_Ge, true);
    d_Go = (static_cast<floatT>(-1.) / mass) * d_Go;
    h_Go = d_Go;
    prop_containers[mass_index].template iterateOverOddBulkAtStack<color1>(
            returnSpinor<floatT, false, Odd, HaloDepthSpin, NStacks>(h_Go));
    ...
    cg.invert(dslash, d_Ge, d_Se, cg_max_iter, cg_residue); //! this takes up most of the computation time
    h_Ge = d_Ge;
    prop_containers[mass_index].template iterateOverEvenBulkAtStack<color2>(
            returnSpinor<floatT, false, Even, HaloDepthSpin, NStacks>(h_Ge));
    dslash.Dslash(d_Go, d_Ge, true);
    d_Go = (static_cast<floatT>(-1.) / mass) * d_Go;
    h_Go = d_Go;
    prop_containers[mass_index].template iterateOverOddBulkAtStack<color2>(
            returnSpinor<floatT, false, Odd, HaloDepthSpin, NStacks>(h_Go));
...
//                returnSpinor<floatT, false, Odd, HaloDepthSpin, NStacks>(h_Go));
    ...
    contract_prop_containers( prop_container )
//    //! contract the prop_containers for all mass combinations. look into the functor "contractPropagators" at the top
//    //! to see what's going on
//    size_t mass_index = 0;
//    for (int m_i = 0; m_i < _n_masses; m_i++) {
